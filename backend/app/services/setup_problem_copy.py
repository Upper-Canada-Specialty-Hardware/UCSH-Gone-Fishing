"""Plain-language title + fix for each setup problem code, for a non-technical reader.

The backend's `detail` carries the specifics of what is wrong with one record;
this is the human "what it means / what to do" that goes with it.

Copied verbatim from frontend/src/components/employeeSetupProblems.ts, which is
what the Employee Setup tab and the per-employee report already show. The nudge
email names the same problems, so it has to name them in the same words - a
problem worded one way on screen and another way in the inbox reads as two
different problems. Codes come from app/services/employee_validation.py; when
either file changes, change both.
"""

PROBLEM_INFO: dict[str, dict[str, str]] = {
    "employee_record": {
